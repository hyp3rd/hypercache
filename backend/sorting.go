...
func (a ByKey) Less(i, j int) bool {
	if a.Asc {
		return a.Items[i].Key < a.Items[j].Key
	}
	return a.Items[i].Key > a.Items[j].Key
}
...
// 	iValue, _ := json.Marshal(a.Items[i].Value)
// 	jValue, _ := json.Marshal(a.Items[j].Value)
// 	if a.Asc {
// 		return string(iValue) < string(jValue)
// 	}
// 	return string(iValue) > string(jValue)
// }
...
// 	}
// 	if a.Asc {
// 		return compare(a.Items[i], a.Items[j])
// 	}
// 	return !compare(a.Items[i], a.Items[j])
// }

type ByLastAccess struct {
	Items []*cache.CacheItem
	Asc   bool
}

func (a ByLastAccess) Len() int {
	return len(a.Items)
}

func (a ByLastAccess) Swap(i, j int) {
	a.Items[i], a.Items[j] = a.Items[j], a.Items[i]
}

func (a ByLastAccess) Less(i, j int) bool {
	if a.Asc {
		return a.Items[i].LastAccess.Before(a.Items[j].LastAccess)
	}
	return a.Items[i].LastAccess.After(a.Items[j].LastAccess)
}

type ByAccessCount struct {
	Items []*cache.CacheItem
	Asc   bool
}

func (a ByAccessCount) Len() int {
	return len(a.Items)
}

func (a ByAccessCount) Swap(i, j int) {
	a.Items[i], a.Items[j] = a.Items[j], a.Items[i]
}

func (a ByAccessCount) Less(i, j int) bool {
	if a.Asc {
		return a.Items[i].AccessCount < a.Items[j].AccessCount
	}
	return a.Items[i].AccessCount > a.Items[j].AccessCount
}
...
func (a ByExpiration) Len() int {
	return len(a.Items)
}

func (a ByExpiration) Swap(i, j int) {
	a.Items[i], a.Items[j] = a.Items[j], a.Items[i]
}

func (a ByExpiration) Less(i, j int) bool {
	if a.Asc {
		return a.Items[i].Expiration < a.Items[j].Expiration
	}
	return a.Items[i].Expiration > a.Items[j].Expiration
}
